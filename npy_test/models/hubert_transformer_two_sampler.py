...
@register_model("hubert_transformer_sampler_two")
...
class HubertTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary=None, embed_tokens=None):
        ...
        if args.conv_kernel_sizes:
            ...
            self.subsampler2 = Conv1dSubsampler(
                args.encoder_embed_dim,
                args.conv_channels,
                args.encoder_embed_dim,
                [int(k) for k in args.conv_kernel_sizes.split(",")],
    ...
    def _forward(self, src_tokens, src_lengths, mode, return_all_hiddens=False):
        if mode == "st":
            ...
            if self.subsampler is not None:
                ...
                x, input_lengths = self.subsampler2(x, input_lengths)
                x = x.transpose(0, 1)  # T x B x C -> B x T x C
...
@register_model_architecture(model_name="hubert_transformer_sampler_two", arch_name="hubert_transformer_sampler_two")
