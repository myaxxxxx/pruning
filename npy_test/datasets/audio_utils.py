...
SF_AUDIO_FILE_EXTENSIONS = {".wav", ".flac", ".ogg", ".npy"}
...
def get_waveform(
    ...
    # waveform = np.load(path_or_fp).astype(np.float32).reshape((1,-1))
    # sample_rate = 16000
...
def get_segment_waveform(
    ...
    # exit()
    # print(path_or_fp)
    waveform = np.load(path_or_fp).astype(np.float32) 
    waveform = torch.from_numpy(waveform)
    waveform = torch.reshape(waveform, (1,-1))
    sample_rate = 16000
