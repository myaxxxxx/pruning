...
@register_model("hubert_transformer_pruning_layer_schedule")
...
class HubertTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary=None, embed_tokens=None):
        ...
        self.hubert_model.encoder.pruning_max_rate = getattr(args, "pruning_max_rate")
        self.hubert_model.encoder.pruning_min_rate = getattr(args, "pruning_min_rate")
    ...
    def _get_hubert_features(self, src_tokens, src_lengths):
        ...
        x, padding_mask, encoder_padding_mask = self.hubert_model.extract_features(**hubert_args)
        output_length = (1 - encoder_padding_mask.int()).sum(dim=1)
    ...
    def _forward(self, src_tokens, src_lengths, mode, return_all_hiddens=False):
        ...
        token_B, token_T, token_C = x.size()
        pad_B, pad_T = encoder_padding_mask.size()
        if token_T != pad_T:
